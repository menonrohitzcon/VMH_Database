...
SET QUOTED_IDENTIFIER ON
GO
CREATE VIEW [dbo].[VW_AllCampaignType]
AS
SELECT 
        CampaignTypes.CampaignTypeId,
		CampaignTypes.DivisionId,
		Divisions.DivisionName,
		CampaignTypes.CampaignType,
		IsNull(CampaignTypes.CampaignTypeCode,'')as CampaignTypeCode,
		IsNull(CampaignTypes.Description,'') as Description,
		IsNull(CampaignTypes.IsActive,0) as IsActive, 

		CampaignTypes.CreatedBy as CreatedById,
		
		isnull(CreatedByUsers.FirstName,'') +' ' + isnull(CreatedByUsers.Prefix,'') + ' ' + isnull(CreatedByUsers.LastName,'') as CreatedBy,

		CampaignTypes.CreatedDateTime,
		CampaignTypes.UpdatedBy as UpdatedById,

		isnull(UpdatedByUsers.FirstName,'')  + ' ' +  isnull(UpdatedByUsers.Prefix,'')  + ' ' + isnull(UpdatedByUsers.LastName,'')  as UpdatedBy,
		CampaignTypes.UpdatedDateTime

from CampaignTypes
	 Left Outer Join Divisions
			ON Divisions.DivisionId = CampaignTypes.DivisionId

	 Left Outer Join Users As CreatedByUsers
			ON CampaignTypes.CreatedBy = CreatedByUsers.UserId

	 Left Outer Join Users As UpdatedByUsers
			ON CampaignTypes.UpdatedBy = UpdatedByUsers.UserId
